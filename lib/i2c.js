...
class I2C {
    ...
    async readRegister(addr, count = 1) {
        ...
        for (let i = 0; i < 1000000; i++) {
            if (this.#device.status.rx_queue_bytes == count + 3) {
                break
            }
        }
        let result = [...await this.#device.read(this.#device.status.rx_queue_bytes)]
    ...
    async readFIFO(addr) {
        ...
        const wr = await this.readRegister(0x04)
        //console.log(rd, wr, wr - rd)
        ...
        if (count > 0) {
            ...
            this.writeByte((this.#addr << 1) | 1)
            for (let i = 0; i < count; i++) {
                this.readByte(i == count - 1? false : true)
            }
            this.setStop()
            await this.#device.write(Uint8Array.from(this.#buf))
            for (let i = 0; i < 1000000; i++) {
